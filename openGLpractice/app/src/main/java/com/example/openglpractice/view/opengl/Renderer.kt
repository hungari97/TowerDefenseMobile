...
import com.example.openglpractice.view.opengl.layer.*
import javax.microedition.khronos.opengles.GL10

class Renderer(activityContext: Context, var presenter: GamePresenter) :
    GLSurfaceView.Renderer {
    private val mActivityContext: Context = activityContext

    val fieldMatrix = presenter.logicFieldMatrix()
    var featureMatrix = presenter.logicFeatureMatrix()
    var characterMatrix = presenter.logicCharacterMatrix()
    ...
    internal val mViewMatrix = Matrix4f(FloatArray(16))
    internal val mProjectionMatrix = Matrix4f(FloatArray(16))
    ...
    override fun onSurfaceCreated(
        ...
        mProgramHandle =
            OShaderHelper.createAndLinkProgram(
                vertexShaderHandle,
        ...
        // Load the texture
        layers.forEach {
            it.mTextureDataHandle = OTextureHelper.loadTexture(mActivityContext, it.textureFile)
        }
    ...
    override fun onSurfaceChanged(glUnused: GL10, width: Int, height: Int) {
        ...
        val near = 1.0f
        val far = 10.0f
        mProjectionMatrix.loadFrustum(left,ratio,bottom,top,near,far)
        ...
        Matrix.setLookAtM(mViewMatrix.array, 0, eyeX, eyeY, eyeZ, lookX, lookY, lookZ, upX, upY, upZ)
    }
    ...
    fun featureUpdate() {
        featureMatrix = presenter.logicFeatureMatrix()
